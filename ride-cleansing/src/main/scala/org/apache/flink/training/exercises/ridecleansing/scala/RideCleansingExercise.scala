...
object RideCleansingExercise {
  ...
  class NYCFilter extends FilterFunction[TaxiRide] {
    override def filter(taxiRide: TaxiRide): Boolean = {
      val rideStartsInNYC = GeoUtils.isInNYC(taxiRide.startLon, taxiRide.startLat)
      val rideEndsInNYC = GeoUtils.isInNYC(taxiRide.endLon, taxiRide.endLat)

      (rideStartsInNYC && rideEndsInNYC)
    }
  }
}
